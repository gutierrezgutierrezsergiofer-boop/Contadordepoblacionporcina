...
def render_caseta(caseta):
    ...
    with st.container(border=True):
        # Fila superior: pasillo + enfermerías (norte)
        # Estructura: [E1] [pasillo] [E2]  -- pero necesitamos 8 columnas de ancho
        # Enfermerías son la mitad del tamaño de un corral
        top = st.columns([0.5, 0.5, 0.5, 3, 3, 0.5, 0.5, 0.5])
        # Ancho total = 8 corrales. Cada corral = 1 unidad.
        # Enfermería = 0.5. Pasillo central = 0.
        # Layout: [E1 (0.5)] [gap] [E2 (0.5)]  centradas arriba
        # Colocamos E1 en col 0 (0.5), E2 en col 7 (0.5)
        # Pero mejor: dividir en 16 columnas (8 por lado), E1 ocupa 0.5
        # Ya se ve: mejor hacerlo simple, dos columnas lado a lado

        # Rehacer: fila superior con E1 y E2 separadas
        c_izq, c_pas, c_der = st.columns([4, 1, 4])
        with c_izq:
            sub = st.columns(8)
            with sub[0]:
                render_unidad(f"{caseta}-E1", estado[f"{caseta}-E1"], compacto=True)
        with c_der:
            sub = st.columns(8)
            with sub[7]:
                render_unidad(f"{caseta}-E2", estado[f"{caseta}-E2"], compacto=True)

        st.markdown("<div style='text-align:center;color:gray'>— pasillo central —</div>",
                    unsafe_allow_html=True)

        # 8 corrales por lado (lado izquierdo: 1-8, derecho: 9-16)
        izq, der = st.columns(2)
        with izq:
            cols = st.columns(8)
            for i in range(1, 9):
                with cols[i-1]:
                    render_unidad(f"{caseta}-{i}", estado[f"{caseta}-{i}"])
        with der:
            cols = st.columns(8)
            for i in range(9, 17):
                with cols[i-9]:
